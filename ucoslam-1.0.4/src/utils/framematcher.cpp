...
std::vector<cv::DMatch> FrameMatcher_Flann::matchEpipolar(const Frame &queryFrame,FrameMatcher::Mode mode,const cv::Mat &FQ2T  ){
    ...
    trainIndex.search(QueryDesc,nn,indices,distances ,xflann::KnnSearchParams(maxSearch,false));
    ...
    for(int i = 0; i < indices.rows; i++) {
        ...
        for(int j=0;j<indices.cols;j++){
            ...
            if ( distances.at<float>(i,j)<bestDist2){
                ...
                if ( !FQ2T.empty())
                if (epipolarLineSqDist(trainKp.pt ,queryKp.pt,FQ2T ) >=3.84*scaleFactor2[queryKp.octave])continue;
