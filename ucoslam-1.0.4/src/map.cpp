...
uint64_t Map::getSignature()const{
    ...
    //cout<<"     Map sig 1. ="<<sig<< " "<<map_points.size()<<endl;
     sig+=keyframes.getSignature();
     //cout<<"     Map sig 2. ="<<sig<<endl;
     for(const auto &m:map_markers){
         sig+=m.first;
         sig+=m.second.getSignature();
     }
     //cout<<"     Map sig 3. ="<<sig<<endl;
     sig+=TheKpGraph.getSignature();
     //cout<<"     Map sig 4. ="<<sig<<endl;
     sig+=TheKFDataBase.getSignature();
     //cout<<"     Map sig 5. ="<<sig<<endl;
     return sig;
...
vector<uint32_t> Map::relocalizationCandidates(Frame &frame){
...
void Map::saveToPcd(std::string filepath,float pointViewDirectionDist)const{
