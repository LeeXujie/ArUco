...
struct UCOSLAM_API Params{
    ...
    float markersOptWeight=0.25;// maximum importance of markers in the final error. Value in range [0,1]. The rest if assigned to points
    ...
    bool autoAdjustKpSensitivity=true;//enables/disables automatic keypoint detector sensitivity to adapt environement with low texture
...
private:
    ...
    //GIVEN THE  nOctaveLevels, scaleFactor and featureDensity, returns maximum number of features per frame to be computed
    int calculateMaxNumberOfFeatures(cv::Size imageSize);
