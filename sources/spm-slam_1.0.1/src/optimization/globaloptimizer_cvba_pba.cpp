...
void GlobalOptimizerCVBA_PBA::setParams(Slam &w,const ParamSet &p,vector<float> *ScaleFactors)throw(std::exception){
    _params=p;
    _params.markerSize=w.markerSize();
    convert(w,_params,_bdata);
}

void GlobalOptimizerCVBA_PBA::convert(const Slam &sl,  ParamSet &p, cvba::BundlerData<float> &bdata)

{
    ...
    //force a camera matrix in which fx=fy
    float meanfxy=(ip.CameraMatrix.at<float>(0,0)+ip.CameraMatrix.at<float>(1,1))*0.5f;
    cameraMatrix.at<float>(0,0)=cameraMatrix.at<float>(1,1)=meanfxy;
    //now, the camera data
    for(auto frameidx:p.used_frames){
        const auto & frame=fset.at(frameidx);
        auto bdata_fidx=frame_index[frameidx];
        cameraMatrix.copyTo(bdata.cameraMatrix[bdata_fidx]);
...
void GlobalOptimizerCVBA_PBA::optimize()throw(std::exception) {
    if (_params.used_frames.size()<2)return;
...
void GlobalOptimizerCVBA_PBA::getResults(Slam &w)throw(std::exception){
...
cameraMatrix.copyTo( w.TheImageParams.CameraMatrix);
...
void GlobalOptimizerCVBA_PBA::optimize(Slam &w,const ParamSet &p )throw(std::exception)
{
...
void GlobalOptimizerCVBA_PBA::saveToStream_impl(std::ostream &str){

}

void GlobalOptimizerCVBA_PBA::readFromStream_impl(std::istream &str)
{
