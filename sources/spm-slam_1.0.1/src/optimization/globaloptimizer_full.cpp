...
namespace ucoslam{
void GlobalOptimizerFull::setParams(Slam &w, const ParamSet &p )throw(std::exception){
...
void GlobalOptimizerFull::convert(Slam &sl,  ParamSet &p, SparseLevMarq<float>::eVector &solution){
   ...
   if (p.fixFirstFrame)  p.fixed_frames.insert(fset.front().idx);
    ...
    solution.resize(pmap_info.size()*3+6*(p.used_frames.size()-p.fixed_frames.size())+6*markermap_info.size() + 9);//last 9 are the camera params
    ...
    for(auto &pi:pmap_info){//for each used map point
        ...
        for(auto f_i:map_point.frames)//each map knows in which frames it projects and the keypoint position in the vector of frame keypoints
            if (p.used_frames.count(f_i.first)){//it is a used frame
                auto undPoint=ip.distortPoint(fset[f_i.first].und_kpts[f_i.second].pt);
                pi.second.frame_projections.push_back(point_prj_info(f_i.first,undPoint ) );
            }
    ...
    for(auto &mi:markermap_info){
        ...
        for(auto fidx:marker.frames)
            if (p.used_frames.count(fidx)){
                mi.second.frame_projections.push_back(marker_prj_info( fidx, ip.distortPoints( fset[fidx].getMarker(mi.first))));
            }
    }
    ...
    //add the camera params
    assert(sl.TheImageParams.Distorsion.total()==5);
    int startCamParams=solution.size()-10;
    solution(startCamParams)=sl.TheImageParams.CameraMatrix.at<float>(0,0);;
    solution(startCamParams+1)=sl.TheImageParams.CameraMatrix.at<float>(1,1);;
    solution(startCamParams+2)=sl.TheImageParams.CameraMatrix.at<float>(0,2);;
    solution(startCamParams+3)=sl.TheImageParams.CameraMatrix.at<float>(1,2);;
    for(int d=0;d<5;d++)
            solution(startCamParams+4+d)=sl.TheImageParams.Distorsion.ptr<float>(0)[d];
...
void GlobalOptimizerFull::optimize(Slam &w, const ParamSet &p )throw(std::exception) {
...
void GlobalOptimizerFull::getMarker3dpoints(const se3 &pose_g2m,float markerSize,vector<cv::Point3f>  &corners){
    corners.resize(4);
    //the corner points
...
void GlobalOptimizerFull::error(const SparseLevMarq<float>::eVector &sol,SparseLevMarq<float>::eVector &err){
    ScopeTimer timer("GlobalOptimizerFull::error");
    ...
    for(const auto &f:_params.used_frames){
        ...
        fp.push_back(getFramePose(sol,f));
    }
    //for each point, project
    for(const auto &p:pmap_info){
        ...
        for(auto &f_p:p.second.frame_projections){
            ...
            err(f_p.errIdx)  =/*_weight_points**/(prj.x-f_p.proj.x);
            err(f_p.errIdx+1)=/*_weight_points**/(prj.y-f_p.proj.y);
        }
    }


    //for each marker, project
    cv::Mat cam=cv::Mat::eye(3,3,CV_32F),dist(1,5,CV_32F);
    int startCamParams=sol.size()-10;
    cam.at<float>(0,0)= sol(startCamParams);
    cam.at<float>(1,1)= sol(startCamParams+1);
    cam.at<float>(0,2)= sol(startCamParams+2);
    cam.at<float>(1,2)= sol(startCamParams+3);

    for(int d=0;d<5;d++)
           dist.ptr<float>(0)[d]= sol(startCamParams+4+d);
    ...
    for(const auto &m:markermap_info){
        ...
        for(auto f_p:m.second.frame_projections){
           // float *rt_t=frame_PrjMatrix[f_p.frame_sol_idx].ptr<float>(0);//get camera projection matrix
            vector<cv::Point2f> prej;
            cv::projectPoints(m_p3d,fp[f_p.frame_sol_idx].getRvec(),fp[f_p.frame_sol_idx].getTvec(),cam,dist,prej);
            for(int c=0;c<4;c++){//project the four points
//                auto prj=fast__project__(rt_t,(float*)&m_p3d[c]);
                err(f_p.errIdx+c*2)  =/*_weight_markers* */(prej[c].x-f_p.proj[c].x);
                err(f_p.errIdx+c*2+1)=/*_weight_markers* */(prej[c].y-f_p.proj[c].y);
            }
        }
    }
}
...
void GlobalOptimizerFull::jacobian(const SparseLevMarq<float>::eVector &sol, Eigen::SparseMatrix<float> &Jac){
    ScopeTimer timer("GlobalOptimizerFull::jacobian");
...
void GlobalOptimizerFull::optimize( )throw(std::exception){
...
//    sl_params.verbose=_params.verbose;
  ...
  //  optimizer.setStepCallBackFunc( std::bind(&GlobalOptimizerFull::StepCallBackFunc,this,std::placeholders::_1));
    if (0)  optimizer.solve( _solution,std::bind(&GlobalOptimizerFull::error,this,std::placeholders::_1,std::placeholders::_2), std::bind(&GlobalOptimizerFull::jacobian,this,std::placeholders::_1,std::placeholders::_2) );
    else optimizer.solve( _solution,std::bind(&GlobalOptimizerFull::error,this,std::placeholders::_1,std::placeholders::_2));
}
void GlobalOptimizerFull::StepCallBackFunc( const SparseLevMarq<float>::eVector  &sol){
    ...
    optimizer.calcDerivates(sol,sJ ,std::bind(&GlobalOptimizerFull::error,this,std::placeholders::_1,std::placeholders::_2));
...
void GlobalOptimizerFull::getResults(Slam &w)throw(std::exception){
    for(const auto &p:pmap_info){
   ...
   //     _debug_msg_(" marker "<<m.first<<" "<<w.TheMap.map_markers[ m.first].pose_g2m<<" "<<pose_g2m);
    ...
    //for each marker, project
    int startCamParams=_solution.size()-10;
    w.TheImageParams.CameraMatrix.at<float>(0,0)= _solution(startCamParams);
    w.TheImageParams.CameraMatrix.at<float>(1,1)= _solution(startCamParams+1);
    w.TheImageParams.CameraMatrix.at<float>(0,2)= _solution(startCamParams+2);
    w.TheImageParams.CameraMatrix.at<float>(1,2)= _solution(startCamParams+3);

    for(int d=0;d<5;d++)
           w.TheImageParams.Distorsion.ptr<float>(0)[d]= _solution(startCamParams+4+d);

}

se3 GlobalOptimizerFull::getFramePose( const SparseLevMarq<float>::eVector &sol,uint32_t frame_idx){
...
void GlobalOptimizerFull::saveToStream_impl(std::ostream &str){

}
void GlobalOptimizerFull::readFromStream_impl(std::istream &str){
