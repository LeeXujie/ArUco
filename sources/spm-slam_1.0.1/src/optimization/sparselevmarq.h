...
#define nanogo_get_time(a,b) std::chrono::duration_cast<std::chrono::duration<T>>(a-b).count()
template<typename T>
bool SparseLevMarq<T>::step( F_z_x f_z_x, F_z_J f_J)throw (std::exception){
    ...
    do{
        ...
        if (gain>0 && ((err-prevErr)<0)){
            dumpFactor=dumpFactor*std::max(T(0.33),T(1.-pow(2*gain-1,3)));
            v=2.f;
            currErr=err;
            curr_z=estimated_z;
            isStepAccepted=true;
        }
        else{ dumpFactor=dumpFactor*v; v=v*5;}


    }while(  gain<=0 && ntries++<5 &&  !isStepAccepted);

    if (_params.verbose) std::cout<<std::setprecision(5) <<"Curr Error="<<currErr<<" AErr(prev-curr)="<<prevErr-currErr<<" gain="<<gain<<" dumping factor="<<dumpFactor<<std::endl;



    if (_params.verbose) {std::cerr<<" J="<<nanogo_get_time(t2,t1)<<" transpose="<< nanogo_get_time(t22,t2)<<" Jt*J="<< nanogo_get_time(t3,t22)<<" B="<< nanogo_get_time(t4,t3) <<" chol="<< nanogo_get_time(t6,t5) <<std::endl;
