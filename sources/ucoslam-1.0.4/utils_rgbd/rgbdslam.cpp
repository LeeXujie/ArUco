...
int main(int argc,char **argv){
    //try {
        ...
        std::shared_ptr<RGBDReader> vcap;
        if (string(argv[1])!="live")
            vcap=RGBDReaderFactory::getReader(argv[2],argv[1]);

        ucoslam::UcoSlam Slam;
        cv::Mat in_image,in_depth;

        ucoslam::ImageParams image_params=getASUSImageParams();
        image_params.CameraMatrix=cv::Mat(cv::Matx33f(vcap->getCamMat()));
        image_params.CamSize=vcap->getImageSize();
        ucoslam::Params params;

         params.aruco_markerSize = stof(cml("-size", "1"));
        params.aruco_DetectorParams.minSize= stod(cml("-marker_minsize", "0.025"));
        params.aruco_DetectorParams.enclosedMarker= cml["-em"];
        ...
        if (cml["-in_debug"]) {//if read debug input, then forget about the rest
            ...
            //read until the last processed frame
                ...
                for(size_t i=0;i<Slam.getLastProcessedFrame();i++) vcap->readNextFrame();
        ...
        while (!finish && vcap->getNextFrame(in_depth,in_image)) {
...
//                Slam.saveToFile("world-"+number+ ".ucs");
        ...
        Slam.saveToFile(cml("-out","world") + ".ucs");
