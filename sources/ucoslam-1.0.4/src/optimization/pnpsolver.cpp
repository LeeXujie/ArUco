...
bool PnPSolver::solvePnPRansac( const Frame &frame, std::shared_ptr<Map> map, std::vector<cv::DMatch> &matches_io, se3 &posef2g_io )
{
    ...
    const float *cam=frame.imageParams.CameraMatrix.ptr<float>(0);
        ...
        cv::Mat rv,tv;
        cv::solvePnP(_p3d,_p2d, frame.imageParams.CameraMatrix,cv::Mat::zeros(1,5,CV_32F),rv,tv,false,CV_P3P);
        //now, count number of inliers of the solution
        //let us project, create the projection matrix
        Se3Transform f2g_ ;f2g_ =(se3(rv,tv).convert());
