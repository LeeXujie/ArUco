...
uint64_t Marker::getSignature() const{
    Hash sig;
    sig+=id;
    sig.add(pose_g2m);
    sig+=size;
...
uint64_t MarkerObservation::getSignature()const{
    Hash sig;
    sig.add(corners.begin(),corners.end());
    sig.add(und_corners.begin(),und_corners.end());
    sig+=ssize;
    sig+=id;
...
void MarkerObservation::draw(cv::Mat& in,  cv::Scalar color, int lineWidth, bool writeId, bool writeInfo) const
{
    ...
    cv::rectangle(in, corners[0] - p2, corners[0] + p2, cv::Scalar(0, 0, 255, 255), -1, CV_AA);
    cv::rectangle(in, corners[1] - p2, corners[1] + p2, cv::Scalar(0, 255, 0, 255), lineWidth, CV_AA);
    cv::rectangle(in, corners[2] - p2, corners[2] + p2, cv::Scalar(255, 0, 0, 255), lineWidth, CV_AA);
