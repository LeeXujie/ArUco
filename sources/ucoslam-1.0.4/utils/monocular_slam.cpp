...
void overwriteParamsByCommandLine(CmdLineParser &cml,ucoslam::Params &params){
    if ( cml["-size"])      params.aruco_markerSize = stof(cml("-size", "1"));
...
int main(int argc,char **argv){
	try {
        ...
        if (argc < 3 || cml["-h"]) {
            cerr << "Usage: (video|live[:cameraIndex(0,1...)])  camera_params.yml [-params ucoslam_params.yml] [-map world]  [-out name] [-vf <float>:video resize factor]"
