...
public:
    ...
    void drawText(cv::Mat &image,const string &userMessage){
        ...
        if (!userMessage.empty()) {
            putText(image,userMessage,cvPoint(30,col));
            col+=20;
        }
        //print help commands
        if(!showingHelp){
            putText(image, "'h' showhelp", cvPoint(30,col));
            putText(image, "'s' start/stop video", cvPoint(30,20+col));
        }
        else{
            ...
            for(auto msg:messages){
                putText(image,msg,cvPoint(30,col));
                col+=20;
            }
